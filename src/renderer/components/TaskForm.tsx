...
import type { Task, Priority, Urgency, RecurrenceType, Subtask } from '../../shared/types';
...
export function TaskForm({ task, onClose, initialDate }: TaskFormProps) {
  const { categories, loadTasks, loadDashboardStats } = useAppStore();
  ...
  useEffect(() => {
    if (task) {
      ...
      setUrgency(task.urgency ?? 'not_urgent');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let savedTask: Task;
      if (task) {
        savedTask = await api.tasks.update({ ...input, id: task.id });
      } else {
        savedTask = await api.tasks.create(input);
      }

      if (reminderAt) {
        await api.reminders.create({ task_id: savedTask.id, remind_at: reminderAt });
      }
      ...
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl border border-border bg-card p-6 shadow-xl animate-scale-in glow-border">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium">Prioridad (Importancia)</label>
              <Select value={priority} onChange={(e) => setPriority(e.target.value as Priority)} options={priorityOptions} />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium">Urgencia</label>
              <Select value={urgency} onChange={(e) => setUrgency(e.target.value as Urgency)} options={urgencyOptions} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium">Fecha límite</label>
              <DateTimePicker value={dueDate} onChange={setDueDate} placeholder="Elegir fecha y hora..." />
            </div>
          </div>

          {/* Reminders */}
          {task ? (
            <ReminderManager taskId={task.id} taskTitle={task.title} />
          ) : (
            <div>
              <label className="mb-1.5 block text-sm font-medium">Recordatorio</label>
              <DateTimePicker value={reminderAt} onChange={setReminderAt} placeholder="Programar recordatorio..." />
            </div>
          )}
          ...
          <div className="rounded-lg border border-border p-3">
            <label className="flex items-center gap-2 text-sm font-medium cursor-pointer">
              <input type="checkbox" checked={isRecurring} onChange={(e) => setIsRecurring(e.target.checked)} className="rounded border-input" />
              Tarea recurrente
            </label>
            {isRecurring && (
              <div className="mt-3 grid grid-cols-2 gap-3">
                <Select value={recurrenceType} onChange={(e) => setRecurrenceType(e.target.value as RecurrenceType)} options={recurrenceOptions} />
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">cada</span>
                  <Input type="number" min={1} max={30} value={recurrenceInterval}
                    onChange={(e) => setRecurrenceInterval(parseInt(e.target.value) || 1)} className="w-16" />
                  <span className="text-sm text-muted-foreground">
                    {recurrenceType === 'daily' ? 'día(s)' : recurrenceType === 'weekly' ? 'sem.' : 'mes(es)'}
                  </span>
                </div>
              </div>
            )}
          ...
          {task && (
            <div className="rounded-lg border border-border p-3">
              ...
              <div className="space-y-1.5 mb-2">
                {subtasks.map((st) => (
                  <div key={st.id} className="flex items-center gap-2 group">
                    <input type="checkbox" checked={!!st.is_completed} onChange={() => handleToggleSubtask(st.id)} className="rounded border-input" />
                    <span className={`flex-1 text-sm ${st.is_completed ? 'line-through text-muted-foreground' : ''}`}>{st.title}</span>
                    <button type="button" onClick={() => handleDeleteSubtask(st.id)} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-red-400">
                      <Trash2 size={12} />
                    </button>
                  </div>
                ))}
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" size="sm" variant="outline" onClick={handleAddSubtask} disabled={!newSubtask.trim()}>
                  <Plus size={14} />
                </Button>
              </div>
            </div>
